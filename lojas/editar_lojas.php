...
if (!isset($_GET['codigo'])) {
    echo "Código da loja não fornecido";
    exit();
}

$codigo = $_GET['codigo'];

$query = "SELECT * FROM Loja WHERE loja_id = '$codigo'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check_query = "SELECT loja_id FROM Loja WHERE nome = '$nome' AND loja_id != '$codigo'";
    $check_result = mysqli_query($link, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        $_SESSION['msg'] = "O nome da loja já está em uso.";
        header("location: editar_lojas.php?codigo=$codigo");
        exit();
    }

    $updateQuery = "UPDATE Loja SET nome = '$nome', cidade_id = '$cidade_id' WHERE loja_id = '$codigo'";
            ...
            <div class="col-md-7 col-lg-5">
                <div class="wrap">
                    ...
                    <div class="login-wrap p-4 p-md-5">
                        ...
                        <p class="text-center"> <a href="ver_lojas.php">Voltar</a></p>
                    </div>
                </div>
            </div>
        </div>
